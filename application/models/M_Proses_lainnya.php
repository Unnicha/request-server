<?php
    ...
    class M_Proses_lainnya extends CI_model {
        ...
        public function getPerMasaTahun($bulan, $tahun) {
            $q = "SELECT * FROM (((proses_lainnya 
                LEFT JOIN tugas ON tugas.id_tugas = proses_lainnya.id_tugas) 
                ...
                ON proses_lainnya.id_kirim = pengiriman_lainnya.id_pengiriman) 
                LEFT JOIN user ON proses_lainnya.id_akuntan = user.id_user) 
        ...
        public function getBelum($bulan, $tahun) {
            $q = "SELECT * FROM (((pengiriman_lainnya 
                ...
                LEFT JOIN (proses_lainnya 
                LEFT JOIN user ON proses_lainnya.id_akuntan = user.id_user) 
                ON proses_lainnya.id_kirim = pengiriman_lainnya.id_pengiriman 
                AND tugas.id_tugas = proses_lainnya.id_tugas) 
                WHERE masa = '$bulan' AND tahun = '$tahun' 
                AND id_proses IS NULL
                ORDER BY pengiriman_lainnya.id_pengiriman ASC"; 
            return $this->db->query($q)->result_array();
        }
