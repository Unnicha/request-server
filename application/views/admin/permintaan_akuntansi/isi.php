		...
		<tbody class="text-center">
			...
			<tr>
				...
				<td>
					...
					<a href="<?= base_url(); ?>admin/permintaan_data_akuntansi/hapus/<?= $a['id_permintaan']; ?>" class="btn btn-sm btn-danger " onclick="return confirm('Yakin ingin menghapus permintaan <?= $a['jenis_data'].' untuk '.$a['nama_klien'] ?>?');" data-toggle="tooltip" data-placement="bottom" title="Hapus">
...
<script>
		...
		$('.btn-detail_permintaan').on('click', function() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url(); ?>admin/permintaan_data_akuntansi/detail',
