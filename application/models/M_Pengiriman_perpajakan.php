<?php
	...
	class M_Pengiriman_perpajakan extends CI_model {
		...
		public function getByMasa($bulan, $tahun, $klien='', $start='', $limit='') {
			if($klien) {
				$this->db->where('permintaan_perpajakan.id_klien', $klien);
			}
			return $this->db->from('pengiriman_perpajakan')
							->join('permintaan_perpajakan', 'permintaan_perpajakan.id_permintaan = pengiriman_perpajakan.id_permintaan', 'left')
							->join('klien', 'permintaan_perpajakan.id_klien = klien.id_klien', 'left')
		...
		public function getPerKlien($bulan, $tahun, $klien) {
			$q = "SELECT * FROM (pengiriman_perpajakan 
				...
				LEFT JOIN klien ON permintaan_perpajakan.id_klien = klien.id_klien) 
				ON permintaan_perpajakan.id_permintaan = pengiriman_perpajakan.id_permintaan) 
				WHERE permintaan_perpajakan.masa = '$bulan' 
				AND permintaan_perpajakan.tahun = '$tahun' 
		...
		public function getMax($id_permintaan) { //fungsi untuk membuat id baru dengan autoincrement
			...
			$q = "SELECT max(id_pengiriman) as maxId FROM pengiriman_perpajakan 
		...
		public function kirim() {
			...
			$redirect = "klien/permintaan_data_perpajakan/kirim/{$id_permintaan}"; //jika terjadi eror kirim
		...
		public function ubahPengiriman() {
			...
			$this->db->update('pengiriman_perpajakan', $data);
		}
