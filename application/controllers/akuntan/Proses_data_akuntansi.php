<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Proses_data_akuntansi extends CI_Controller {
		...
		public function prosesOn() {
			...
			$this->load->view('akuntan/proses_akuntansi/view_'.$_POST['tab'], $data);
		}
		...
		public function gantiKlien() {
			...
			if($akses) {
				...
				if($akses) {
					$klien = $this->Klien_model->getById(explode(',', $akses['akuntansi']));
				}
			}
			...
			foreach($proses as $k) {
				...
				if($status == 'todo') {
					...
					$btnSelesai = ($k['id_akuntan'] != $akuntan) ? '' : '
						<a href="proses_data_akuntansi/done/'.$k['id_proses'].'" data-toggle="tooltip" data-placement="bottom" title="Selesaikan Proses">
		...
		public function detail() {
			$proses	= $this->M_Proses->getByProses($this->input->post('id', true));
		...
		// 		redirect('akuntan/proses_data_akuntansi?p=export');
		// 	}
		// }
	}
?>
