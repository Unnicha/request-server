...
<script>
	$(document).ready(function() {
		function klien() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url(); ?>akuntan/penerimaan_data_lainnya/klien',
		...
		var table = $('#myTable').DataTable({
			...
			//'pageLength': 9,
			'ajax'		: {
				'url'	: '<?=base_url()?>akuntan/penerimaan_data_lainnya/page',
				'type'	: 'post',
				'data'	: function (e) { 
					e.klien = $('#klien').val(); 
					e.bulan = $('#bulan').val(); 
		...
		$('#myTable tbody').on('click', 'a.btn-detail_pengiriman', function() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= base_url(); ?>akuntan/penerimaan_data_lainnya/detail',
