<?php
	...
	class M_Proses_lainnya extends CI_model {
		...
		public function simpanProses( $data ) {
			...
			$this->db->insert('proses_lainnya', $row1);
			
			$row2 = [
				'status_proses' => ( $data['tanggal_selesai'] ) ? 'done' : 'yet',
				// 'status_proses' => $data['tanggal_selesai'],
			];
			$this->db->update('data_lainnya', $row2, ['id_data' => $data['kode_data']]);
		...
		public function ubahProses( $data ) {
			...
			$this->db->update('proses_lainnya', $row1, ['id_proses' => $data['id_proses']]);
		...
		public function batalProses( $data ) {
			...
			$this->db->update('data_lainnya', $row, ['id_data' => $data['kode_data']]);
		}
	}
?>