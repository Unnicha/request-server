	...
	class Penerimaan_data_perpajakan extends CI_Controller {
		...
		public function page() {
			...
			foreach($pengiriman as $k) {
				$badge = []; $date = [];
				$detail = $this->M_Pengiriman_perpajakan->getDetail($k['id_permintaan']);
				foreach($detail as $d) {
					if($d['status'] == 1) {
		...
		public function konfirmasi($id_pengiriman) {
			...
			$isi		= $this->M_Pengiriman_perpajakan->getDetail($pengiriman['id_permintaan']);
			...
			$data['judul']			= "Konfirmasi Pengiriman - Data Perpajakan"; 
			...
			if($this->form_validation->run() == FALSE) {
				$this->libtemplate->main('akuntan/penerimaan_perpajakan/konfirmasi', $data);
			} else {
				if($this->M_Pengiriman_perpajakan->konfirmasi() == 'OK') {
					$this->session->set_flashdata('notification', 'Data berhasil dikonfirmasi!');
				}
				redirect('akuntan/penerimaan_data_perpajakan');
			}
		}
		...
		public function detail() {
			...
			$this->load->view('akuntan/penerimaan_perpajakan/detail', $data);
		}
	}
?>