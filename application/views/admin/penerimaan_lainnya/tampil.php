<div class="container-fluid"> 
	...
	<!-- Form Ganti Tampilan -->
	<form action="<?=base_url()?>admin/penerimaan/penerimaan_data_lainnya/cetak" method="post" target="_blank">
...
<script>
	$(document).ready(function() { 
		var table = $('#myTable').DataTable({
			...
			//'pageLength': 9,
			'ajax'		: {
				'url'	: '<?=base_url()?>admin/penerimaan/penerimaan_data_lainnya/page',
				'type'	: 'post',
				'data'	: function (e) { 
					e.klien = $('#klien').val(); 
					e.bulan = $('#bulan').val(); 
		...
		$('#myTable tbody').on('click', 'a.btn-detail_pengiriman', function() {
			...
			$.ajax({
				type	: 'POST',
				url		: '<?= base_url(); ?>admin/penerimaan/penerimaan_data_lainnya/detail',
