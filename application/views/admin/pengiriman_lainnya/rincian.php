...
<script>
	$(document).ready(function () {
		...
		$('.btn-konfirm').click(function() {
			$.ajax({
				type	: 'post',
				url		: '<?= base_url(); ?>admin/pengiriman/pengiriman_data_lainnya/konfirmasi',
				data	: {
					'id'		: $(this).data('id'),
		...
		$('#konfirmShow').on('click', '.btn-fix', function() {
			...
			$.ajax({
				type	: 'post',
				url		: '<?= base_url(); ?>admin/pengiriman/pengiriman_data_lainnya/fix',
				data	: {
					'id'	: id,
					'stat'	: stat
				},
				success	: function() {
					...
					window.location.assign("<?= base_url();?>admin/pengiriman/pengiriman_data_lainnya/detail/"+id);
				}
			})
		})
	})
