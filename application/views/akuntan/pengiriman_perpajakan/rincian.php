...
<script>
	$(document).ready(function () {
		...
		$('.btn-konfirm').click(function() {
			$.ajax({
				type	: 'post',
				url		: '<?= base_url(); ?>akuntan/pengiriman_data_perpajakan/konfirmasi',
				data	: {
					'id'		: $(this).data('id'),
		...
		$('#konfirmShow').on('click', '.btn-fix', function() {
			...
			$.ajax({
				type	: 'post',
				url		: '<?= base_url(); ?>akuntan/pengiriman_data_perpajakan/fix',
				data	: {
					'id'	: id,
					'stat'	: stat
				},
				success	: function() {
					...
					window.location.assign("<?= base_url();?>akuntan/pengiriman_data_perpajakan/detail_pengiriman/"+id);
				}
			})
		})
	})
