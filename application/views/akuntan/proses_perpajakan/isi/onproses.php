	...
	<table class="table table-sm table-striped table-bordered table-responsive-sm mb-3" id="isiProses">
		...
		<tbody class="text-center">
			...
			<tr>
				...
				<td><?= $standar ?></td>
				<td>
					<a class="btn btn-sm btn-warning" href="<?= base_url(); ?>akuntan/proses_data_perpajakan/selesai/<?=$a['id_proses']?>" data-toggle="tooltip" data-placement="bottom" title="Selesaikan Proses">
...
<script>
	$(document).ready(function() {
		...
		function durasi(mulai) {
			$.ajax({
				type: 'POST',
				url: '<?=base_url()?>akuntan/proses_data_perpajakan/hitung_durasi',
				data: {mulai:mulai},
				async: false,
				dataType: 'html',
