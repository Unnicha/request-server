<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Proses_data_perpajakan extends CI_Controller {
		...
		public function prosesOn() {
			...
			$this->load->view('akuntan/proses_perpajakan/view_'.$_POST['tab'], $data);
		}
		...
		public function gantiKlien() {
			...
			if($akses) {
				...
				if($akses) {
					$klien = $this->Klien_model->getById(explode(',', $akses['perpajakan']));
				}
			}
		...
		public function page() {
			...
			foreach($proses as $k) {
				...
				if($status == 'belum') {
					...
					$row[]	= $standar;
					$row[]	= '
						<a href="proses_data_perpajakan/mulai/'.$k['id_proses'].'" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="bottom" title="Mulai Proses">
		...
		public function mulai($id_proses) {
			...
			if($this->form_validation->run() == FALSE) {
				...
				if($this->m_proses->tambahProses() == 'ERROR') {
					redirect('akuntan/proses_data_perpajakan/mulai/'.$id_proses);
				} else {
		...
		public function download() {
			...
			if($proses) {
				...
				redirect('akuntan/proses_data_perpajakan?p=export');
			}
		}
	}
?>
